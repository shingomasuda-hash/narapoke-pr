...
export const shop = {
  ...
  /**
   * Google タグマネージャーのコンテナID
   * ------------------------------------------------------------
   * ここに ID を入れると、全ページにタグマネージャーが読み込まれます。
   * 空にすると一切読み込まれません（検証環境で計測したくないときなど）。
   *
   * 環境変数 NEXT_PUBLIC_GTM_ID を設定すれば、そちらが優先されます。
   * 本番だけ計測したい場合は、ここを空にして本番環境の環境変数だけに入れてください。
   */
  gtmId: process.env.NEXT_PUBLIC_GTM_ID || "GTM-PQRDCS8X",
};
